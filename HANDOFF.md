...
- 백엔드: Privy 검증, Pinata, `after()` 파이프라인, 스위퍼/내부 라우트가 구현돼 최종 통합 중
- 운영자: 닉네임 수정, DB·Storage 초기화, `capabilities`, 세션 만료 처리가 구현돼 최종 통합 중
- 문서: 현재 코드 계약과 운영 배포 절차를 최종 동기화 중
...
1. `git status`와 에이전트 결과를 통합하고 위 네 검증 명령을 통과시킨다.
2. 현재 백엔드·운영자·문서 변경을 목적별 커밋으로 나눠 PR #10에 푸시한다.
3. 참가자 화면에 실제 Privy Bearer token과 공용 프레임 합성을 연결한다.
4. Vercel 환경변수와 1분 Cron(`/api/internal/sweep`)을 설정해 배포한다.
5. `after()`를 포함한 한 명 end-to-end 리허설 뒤 DB·Storage를 비운다.
6. 행사 직전 Supabase 상태, 민터 권한·잔액, 운영자 로그인과 TV를 확인한다.
