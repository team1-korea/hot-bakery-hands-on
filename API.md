...
1. 이 문서 1절(역할 경계) — 무엇이 내 몫인지
2. [`contracts/INTEGRATION_GUIDE.md`](./contracts/INTEGRATION_GUIDE.md) — 컨트랙트를 어떻게 부르는지.
   민팅 코드를 쓸 때 이 문서가 정본입니다
3. 이 문서 2·3절 — 프론트가 부르는 경로와 상태 전이
4. 이 문서 9절 체크리스트 — 빠뜨린 것 확인

**만드는 순서**

프론트는 이미 동작합니다. 목 서버를 하나씩 실제 구현으로 바꿔 나가는 방식이 가장 빠릅니다.

1. 상태와 사진을 저장할 곳을 정한다(DB + 오브젝트 스토리지)
2. `POST /api/entries`와 `GET /api/state`를 실제 저장소로 만든다 → 이 둘만 되면 TV가 살아납니다
3. 로그인 두 경로를 실제 메일 발송으로 바꾼다
4. 증서 메타데이터를 만들어 IPFS에 올린다(4절에서 형태를 먼저 확정)
5. `mint`를 부르고 영수증·이벤트를 확인해 `MINTED`로 올린다
6. 실패를 `FAILED`로 기록하고 `retry`를 받는다

**프론트와 붙여 보는 방법**

`apps/web/.env.local`에 백엔드 주소를 넣으면 화면이 그쪽을 부릅니다. 목 라우트는 지우지 않아도
호출되지 않습니다.
...
`/join`으로 참가자 흐름, `/display`로 행사장 TV, `/admin`으로 운영자 화면을 열어 확인합니다.
운영자 화면은 `OPERATOR_PASSCODE`가 필요합니다(6절).
