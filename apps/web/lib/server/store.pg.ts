 ...
 */
/**
 * 차례를 못 잡고 이만큼 지나면 포기한다.
 *
 * 라우트 수명이 `maxDuration = 60`인데 앞 단계가 이미 예산을 먹는다 — 사진 핀 최대 12초,
 * 메타데이터 핀 최대 12초, 전송과 영수증에 20여 초. 45초를 기다리면 **포기를 기록하기도 전에
 * Vercel이 인보케이션을 죽인다.** 그러면 행이 중간 상태로 남아 운영자 화면에서 정상 카드와
 * 구별되지 않고, 스위퍼로 90초를 더 기다려야 풀린다.
 *
 * 25초면 남은 예산 안에서 스스로 `FAILED`가 될 수 있다. **실패는 「실패」 카운터에 잡히고
 * 재시도 버튼이 붙지만, 죽어서 멈춘 행은 아무 신호도 내지 않는다.**
 */
const MINT_LOCK_WAIT_MS = 25_000;
