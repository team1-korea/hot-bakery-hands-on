 ...
 *
 * **모든 export가 Promise를 돌려준다.** 지금은 기다릴 것이 없지만 Postgres로 갈아탈 때
 * 라우트를 한 줄도 안 고치기 위해서다. `db/schema.sql`이 이 파일이 흉내내는 대상이다.
 */
...
export async function attachPhoto(
  ...
  store.photos.set(row.entry.id, photo);
  row.entry.photoUrl = `/api/photos/${row.entry.id}`;
...
export async function sweep(now: number = Date.now()): Promise<{ failed: number; hidden: number }> {
  ...
  for (const row of store.rows) {
    ...
    if (OVEN_STATUSES.includes(row.entry.status) && now - row.statusChangedAt >= STUCK_MS) {
      row.entry.failureReason = '처리 중 멈춤 (스위퍼)';
