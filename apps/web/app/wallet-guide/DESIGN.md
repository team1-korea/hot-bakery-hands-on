...
빨간 브랜드 띠에는 4칸 진행선을 두고, 현재 펼친 티켓과 같은 순번의 칸만 종이색으로 밝힌다. 영웅 영역은 위 38px 여백 뒤에 제목과 최대 39ch의 한 문단을 놓고, 30px 뒤부터 티켓 묶음을 시작한다. 티켓은 20px의 종이 간격으로 이어지고, 펼친 본문은 좌우 22px로 들여쓴다. 하단 안전 고지는 44px 간격과 4px 상단 규칙선으로 본문에서 분리한다.
...
- **Shape:** 2px 외곽선과 직각 모서리, 위아래의 삼각 절취 톱니, 번호·조작 열의 1px 점선으로 실제 작업 티켓처럼 구성한다.
...
### Anchored Ticket Transition

- **Behavior:** 다른 티켓을 누를 때 위쪽 티켓이 접혀도 사용자가 누른 티켓 머리의 화면상 위치를 유지한다. 다음 단계 버튼으로 이동할 때는 포커스를 스크롤 없이 먼저 옮기고, 화면 밖에 있을 때만 가장 가까운 위치까지 부드럽게 이동한다.
