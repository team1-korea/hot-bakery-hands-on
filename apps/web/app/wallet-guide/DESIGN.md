...
description: "참가증서 확인을 먼저 제공하고 원하는 참가자만 Privy 보안 창을 통해 Core 지갑 연결을 진행하는 모바일 우선 안내"
colors:
...
이 페이지의 첫 번째 성공은 참가자가 개인키 없이 OpenSea에서 자신의 증서를 확인하는 것이다. Core Wallet 가져오기는 기본 절차가 아니라 직접 지갑을 관리하려는 사람만 선택하는 고급 절차다. 원하는 참가자는 행사 때 사용한 Google 계정으로 인증하고 Privy 공식 보안 창을 직접 열 수 있다. 앱은 개인키 값을 읽거나 저장하지 않으며, 페이지 안에 개인키 입력란이나 예시 값을 만들지 않는다.
...
4. Core 모바일 설치
5. 안내 메일을 받은 Google 계정으로 본인 확인
6. 서버에서 `MINTED` 참가증서 기록과 현재 Privy 지갑 주소의 일치 확인
7. Privy 공식 보안 창과 개인키 위험 설명
8. Core 가져오기 과정 설명
9. 같은 EVM 주소와 Collectibles 확인
...
### Participant-controlled Wallet Export

- 개인키를 가진 사람이 지갑을 사용할 수 있다는 결과를 설명한다.
- 이메일, 메신저, 메모장에 보내거나 저장하지 말라고 행동 가까이에서 알린다.
- 참가자가 행사 때 사용한 Google 계정으로 직접 인증한 뒤 Privy 공식 보안 창을 연다.
- 서버에서 인증된 DID의 `MINTED` 기록과 현재 Privy 임베디드 EVM 주소가 일치한 계정에만 지갑 주소와 내보내기 버튼을 보여 준다.
- 발급 기록이 없으면 안내 메일을 받은 Google 계정인지 확인하도록 안내하고, 새로 생긴 빈 지갑을 행사 지갑처럼 표시하지 않는다.
- 페이지와 서버는 개인키 값을 읽거나 저장하지 않는다.
- 페이지에는 개인키 입력란, 일부 값, QR 코드, 가짜 개인키 예시를 만들지 않는다.
- 내보내기 전후에 참가자의 Privy EVM 주소와 Core EVM 주소가 같은지 확인하게 한다.
