...
description: "참가증서 확인을 먼저 끝내고 Core 지갑 관리는 선택적으로 안내하는 모바일 우선 읽기 화면"
colors:
...
이 페이지의 첫 번째 성공은 참가자가 개인키 없이 OpenSea에서 자신의 증서를 확인하는 것이다. Core Wallet 가져오기는 기본 절차가 아니라 직접 지갑을 관리하려는 사람만 선택하는 고급 절차다. 따라서 첫 화면은 증서 확인을 강조하고, 개인키 안내는 충분한 맥락과 위험 확인 뒤에만 행동할 수 있게 한다.
...
4. Core 모바일 설치
5. 행사 Google 계정 로그인
6. 위험 확인 뒤 Privy 공식 내보내기 창을 열고 Core에 즉시 가져오기
...
### Login and Wallet Proof

- 행사 때 사용한 Google 계정으로만 로그인하게 한다.
- 로그인 이메일과 축약 EVM 주소를 보여 주되 개인키는 렌더링하지 않는다.
- 여러 Privy 지갑이 있으면 백엔드와 같은 기준으로 대상 지갑을 선택한다.

### Security Gate

- 개인키를 가진 사람이 지갑을 사용할 수 있다는 결과를 설명한다.
- 이메일, 메신저, 메모장에 보내거나 저장하지 말라고 행동 가까이에서 알린다.
- 참가자가 위험 확인 체크박스를 선택하기 전에는 Privy 내보내기 버튼을 비활성화한다.
...
- 주요 행동, 체크박스, 계정 전환에는 뚜렷한 포커스 링을 제공한다.
- 비활성화된 내보내기 버튼은 로그인, 지갑 주소, 위험 확인 중 무엇이 필요한지 인접 문구로 설명한다.
