...
락을 잡은 한 건만 민팅을 진행합니다. 못 잡은 인보케이션은 트랜잭션을 바로 끝내 DB 커넥션을
반납하고 400~1000ms 뒤 다시 시도합니다. 이 대기를 최대 20초까지만 반복하므로 뒤에 선 참가자들이
커넥션 풀을 점유하지 않고, Vercel 60초 제한 안에서 실패를 DB에 기록할 시간도 남습니다. 20초 안에
차례를 못 잡으면 `FAILED`로 내려 운영자 재시도로 다시 시작합니다.
...
재시도하면 `PINNED`부터 새 트랜잭션을 보냅니다. 영수증이 없으면 트랜잭션 전송 후 5분까지는 무조건
보류합니다. 그 뒤에도 공개 RPC에서 400ms 간격으로 세 번 연속 트랜잭션을 찾지 못했을 때만
해시를 비웁니다. 한 번이라도 보이거나 RPC 조회 자체가 실패하면 pending 트랜잭션을 중복 전송하지
않도록 기존 해시를 유지하고 다음 점검으로 미룹니다. 해시를 보존한 실패와 운영자 재시도는
`status_changed_at`도 최초 전송 시각으로 유지해 5분 유예가 다시 시작되지 않게 합니다.
